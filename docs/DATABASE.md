...
profil Maven actif, comme avant cette évolution. Le paramètre
`INIT=RUNSCRIPT FROM 'classpath:import.sql'` reste inchangé dans les 3 cas :
`import.sql` s'exécute normalement à la première connexion, quel que soit le
fichier H2 ciblé.
